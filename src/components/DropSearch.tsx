...
export default (props: Selector) => {
  ...
  const handleSearch = (e: any) => {
    ...
    const handleAlert = () => {
      if (listboxRef.current && listboxRef.current.offsetHeight < 5) {
        if (alrtEl) alrtEl.classList.remove("hidden");
      } else {
        if (alrtEl) alrtEl.classList.add("hidden");
      }
    };
    handleAlert();
  ...
  useEffect(() => {
    document.onclick = (e: any) => {
      ...
      if (target) if (!target.closest(".label-button")) setState(false);
    };
  }, []);
    ...
    <div className="relative text-base">
      <div className="label-button flex items-center gap-1 px-2 border rounded-lg shadow-sm">
        ...
        <input
          ...
          className="w-full px-2 py-3 text-gray-500 bg-transparent rounded-md outline-none"
        ...
        {searchFieldVal ? (
          <button
            onClick={() => {
              setSearchFieldVal("");
              props.setSelectedItem({ item: "", idx: null });
              setState(false);
            }}
          >
            <svg
          ...
          >
            ...
            {menuItems.map((el, idx) => (
              <li
                key={idx}
                onClick={() => {
                  props.setSelectedItem({ item: el, idx });
